...
import thu.declan.xi.server.model.Withdraw.Channel;
...
//		}
		...
		try {
			...
			if(oldWd.getState() == WState.REFUSED){ // 之前被拒绝的
				...
				// 初审通过
				withdrawService.update(updater); // 通过 减去金额
			}
			...
			if (updater.getState() == WState.PASSED || updater.getState() == WState.PAID) { // 通过申请
				Notification noti = notiService.addNoti(acc.getId(), Notification.NType.WITHDRAW, withdrawId, 
						Notification.TPL_WITHDRAW); // 提示提现到账
	...
	private void againEditWithdraw(Withdraw wit) throws ApiException {
		...
		try {
			...
			try {
				wit.setState(WState.AGAINREFUSED);
				withdrawService.update(wit);
	...
	public void paymentPayroll(@Valid Payroll pay) throws ApiException {
		LOGGER.debug("==================== enter WithdrawResource paymentPayroll ====================");
		Integer auditId = currentAccountId();
		LOGGER.debug("payrollId: " + pay.getId() + " auditId: " + auditId);
		SalaryTransit sTransit = new SalaryTransit();
		Withdraw with = new Withdraw();
		List<SalaryTransit> list = null;
		List<SalaryTransit.State> states = new ArrayList<SalaryTransit.State> ();
		states.add(SalaryTransit.State.SALARY_ERROR);
		states.add(SalaryTransit.State.SUCCESS);
		sTransit.setPayId(pay.getId());
		sTransit.setQueryStates(states);
		try {
			list = salaryTransitService.getList(sTransit);
			sTransit.setQueryStates(null);
			for (SalaryTransit st : list) {
				st = payment(st, with, st.getAccount());
				st.setAuditId(auditId);
				st.setPaymentDate(new Date());
				salaryTransitService.update(st);
			}
			Integer total_number = salaryTransitService.getCount(sTransit);
			sTransit.setState(State.SALARY_ERROR);
			Integer error_number = salaryTransitService.getCount(sTransit);
			if (error_number != 0 && total_number == error_number) {
				pay.setPayState(PaState.ERROR);
			} else if (error_number != 0 && total_number > error_number) {
				pay.setPayState(PaState.SUCCESS_PART);
			} else {
				pay.setPayState(PaState.SUCCESS_ALL);
			}
			payrollService.update(pay);
		} catch (ServiceException e) {
			String devMsg = "Service Exception [" + e.getCode() + "] " + e.getReason();
            LOGGER.debug(devMsg);
			handleServiceException(e);
		}
		LOGGER.debug("==================== leave WithdrawResource paymentPayroll ====================");
	}
	
	@POST
	@Path("/payment/salaryTransit")
	@Consumes(MediaType.APPLICATION_JSON)
	@RolesAllowed({Constant.ROLE_ADMIN})
	public void paymentSalaryTransit(@Valid SalaryTransit st) throws ApiException {
		LOGGER.debug("==================== enter WithdrawResource paymentSalaryTransit ====================");
		Integer auditId = currentAccountId();
		LOGGER.debug("salaryTransitId: "+ st.getId() +" audditId: "+ auditId);
		SalaryTransit sTransit = new SalaryTransit();
		SalaryTransit salaryTransit = new SalaryTransit();
		Account account = new Account();
		Withdraw with = new Withdraw();
		Payroll payroll = new Payroll();
		try {
			sTransit = salaryTransitService.get(st.getId());
			account = accountService.get(sTransit.getAccId());
			st = payment(sTransit, with, account);
			st.setAuditId(auditId);
			st.setPaymentDate(new Date());
			salaryTransitService.update(st);

			payroll.setId(sTransit.getPayId());
			salaryTransit.setPayId(sTransit.getPayId());
			Integer total_number = salaryTransitService.getCount(salaryTransit);
			salaryTransit.setState(State.SALARY_SUCCESS);
			Integer success_number = salaryTransitService.getCount(salaryTransit);
			salaryTransit.setState(State.SALARY_ERROR);
			Integer error_number = salaryTransitService.getCount(salaryTransit);
			if (success_number != 0 && total_number == success_number && error_number == 0) {
				payroll.setPayState(PaState.SUCCESS_ALL);
			} else if (success_number == 0 && total_number == error_number) {
				payroll.setPayState(PaState.ERROR);
			} else{
				payroll.setPayState(PaState.SUCCESS_PART);
			}
...
//			e.printStackTrace();
		...
		LOGGER.debug("==================== leave WithdrawResource paymentSalaryTransit ====================");
	}
	


	private SalaryTransit payment(SalaryTransit st, Withdraw with, Account account) {
		LOGGER.debug("==================== enter WithdrawResource payment ====================");
		Integer accId = st.getAccId();
		Double total = st.getSubTotal();
		String openId = account.getOpenId();
		Integer payId = st.getPayId();
		Payroll payroll = null;
		LOGGER.debug("accountId: " + accId + " money: " + total + " openId: " + openId + " payrollId: "+ payId);
		st.setState(State.SALARY_ERROR);
		if (account != null && openId != null) { // 已绑定微信 可付款至零钱
			with.setAccountId(accId);
			with.setState(WState.PASSED); // 设置状态通过
			with.setChannel(Channel.WECHAT); // 设置付款渠道 
			with.setValue(total); // 设置金额
			try {
				if (payId != null) {
					payroll = payrollService.get(payId);
				}
				withdrawService.insert(with);
				withdrawService.payWages(with.getId(), account, openId, total, payroll.getMonth());
				st.setState(State.SALARY_SUCCESS);
				
				
				Notification noti = notiService.addNoti(accId, Notification.NType.SALARY, with.getId(), true,
						Notification.TPL_SALARY_OUT, payroll.getComName(), payroll.getMonth(), st.getSubTotal()); // 提示付款到账
				Map<String, String> data = new HashMap<>();
				data.put("first", "您好!您的薪资已到账");
				data.put("keyword1", String.format("%.2f", total));
				data.put("keyword2", (new SimpleDateFormat("YYYY-MM-dd HH:mm")).format(new Date()));
				data.put("remark", "感谢您的使用");
				if (openId != null) {
					try { // 微信发送提示消息
				...
				ex.printStackTrace();
				st.setPrompt(ex.getReason());
			} 
		} else {
			st.setPrompt("该用户未绑定微信，无法发放工资。");
		}
		LOGGER.debug("==================== leave WithdrawResource payment ====================");
		return st;
	}
