...
public class WithdrawServiceImpl extends BaseTableServiceImpl<Withdraw> implements WithdrawService {
	...
	@Override
	public void payWages(Integer id, Account acc, String openId, Double money, String month) throws ServiceException {
		Withdraw withdraw = get(id);
		if (acc == null || openId == null || acc.getRole() != Account.Role.STUDENT) { // 判断付款对象角色
			throw new ServiceException(ServiceException.CODE_VERIFY_FAILED, "账户角色错误");
		}
		Student stu = studentMapper.selectByAccountId(acc.getId());
		if (stu == null || stu.getName() == null) {
			throw new ServiceException(ServiceException.CODE_VERIFY_FAILED, "学生信息错误");
		}
		wechatService.transformation(openId, stu.getName(), id, money, month); // 调用新的付款功能
		withdraw.setState(Withdraw.WState.PAID); // 修改状态
		withdraw.setPayTime(new Date()); // 设置付款时间
		withdrawMapper.update(withdraw);
	}
