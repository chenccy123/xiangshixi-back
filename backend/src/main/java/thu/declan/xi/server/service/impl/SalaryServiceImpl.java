...
public class SalaryServiceImpl extends BaseTableServiceImpl<Salary> implements SalaryService {
	...
	@Override
	public void insertSalary(Salary salary) throws ServiceException {
		Student stu = studentMapper.selectOne(salary.getStuId());
		if (stu == null) {
			throw new ServiceException(ServiceException.CODE_NO_SUCH_ELEMENT, "No such student");
		}
		accountMapper.addBalance(stu.getAccountId(), salary.getStuId());
		salary.setPayTime(new Date());
		salary.setState(Salary.SState.PAID);
