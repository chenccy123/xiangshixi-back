...
//	private static final Logger LOGGER = LoggerFactory.getLogger(BaseTableServiceImpl.class);
	...
	private String create_nonce_str() { // 随机字符串
	...
	public void transfer(String openid, String checkname, int tradeNo, double value) throws ServiceException {
		...
		trans.setAmount(String.format("%.0f", value*100)); // 金额  企业付款金额，单位为分
//		trans.setCheck_name("FORCE_CHECK");
		trans.setCheck_name("NO_CHECK"); // 校验用户姓名  NO_CHECK：不校验真实姓名   FORCE_CHECK：强校验真实姓名
		trans.setDesc("提现"); // 付款描述
		trans.setMch_appid(Constant.WECHAT_APPID); // 商户账号appid 付款appid
		trans.setMchid(Constant.WECHAT_MCH_ID); // 商户号
		trans.setNonce_str(nonce); // 随机字符串  必填
		trans.setOpenid(openid); // 用户openid 收款人openid
		trans.setPartner_trade_no(String.format("%d", tradeNo)); // 商户订单号 
		trans.setRe_user_name(checkname); // 收款用户姓名
		trans.setSpbill_create_ip(Constant.SERVER_IP); // Ip地址
...
//		trans.setSign(sign);
		...
		if (result.getErr_code() != null) {
			throw new ServiceException(ServiceException.CODE_EXTERNAL_ERROR, result.getErr_code_des());
		}
	}

	@Override
	public void transformation(String openid, String checkname, int tradeNo, double value, String month) throws ServiceException {
		Transfers trans = new Transfers();
		String nonce = create_nonce_str().substring(0, 32);
		trans.setAmount(String.format("%.0f", value*100)); // 企业付款金额，单位为分
		trans.setCheck_name("NO_CHECK"); // 校验用户姓名  NO_CHECK：不校验真实姓名   FORCE_CHECK：强校验真实姓名
		trans.setDesc(month + "月工资"); // 款项描述
		trans.setMch_appid(Constant.WECHAT_APPID); // 商户账号appid 付款appid
		trans.setMchid(Constant.WECHAT_MCH_ID); // 商户号
		trans.setNonce_str(nonce); // 随机字符串  必填
		trans.setOpenid(openid); // 用户openid 收款人openid
