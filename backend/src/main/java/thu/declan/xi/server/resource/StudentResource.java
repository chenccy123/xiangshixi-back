...
//			@QueryParam("schoolType") SchoolType schoolType,
...
//		ListResponse<Student> res = getStudents(pageIndex, pageSize, schoolName, name, phone, frozen);
		...
		for (Student stu : res.getItems()) {
			...
			d.put("schoolType", stu.getSchoolType() == null ? "" : stu.getSchoolType().toChsString());
//			d.put("schoolName", stu.getSchoolName());
		...
		titles.put("email", "邮箱");
		titles.put("area", "地区");
		titles.put("school", "学校名称");
		titles.put("schoolType", "学校类别");
//		titles.put("schoolName","学院名称");
		titles.put("edu", "学历");
		titles.put("major", "专业");
		titles.put("grade", "年级");
		titles.put("lang", "外语");
...
//			}
	...
	public ListResponse<Rate> getStudentRates(@PathParam("studentId") int studentId,
		...
		selector.setDirection(Rate.Direction.COMP_TO_STU); // 获取企业评价信息
